...
export default function CategoriesFilter({ products }: { products: Product[] }) {
    ...
    const handlePriceChange = (newPrice: number, hide: boolean = false) => {
        setIsLoading(true);
        setPrice(newPrice);
        hide ? setIsClear(false) : setIsClear(true);
            ...
            <>
                ...
                {isClear && (
                    <div className="flex justify-end">
                        <span className="text-sm text-gray-500 cursor-pointer border border-gray-300 hover:bg-gray-50 hover:border-blue-500 rounded-md p-1 mb-2"
                            onClick={() => handlePriceChange(PRICE_RANGE.min, true)}>{t('clear')}</span>
                    </div>
                )}
